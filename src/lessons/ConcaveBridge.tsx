...
import { calculateConcaveBridge } from '../physics/circular';
import { formatPhysicsValue } from '../physics/mechanics';

const ConcaveBridge: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const {
    ...
    weightRatio,
    statusText,
  } = calculateConcaveBridge(mass, speed, radius);
  ...
  const bridgeCenterY = 600 - rPx; // Concave bridge center is above
  
  // Object position
  const objX = CENTER_X;
  const objY = 600;
  
  const clip = (v: number) => Math.min((v / weight) * 100, 200); // Scale arrows relative to weight
  const W = 80;
  const H = 40;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '1.5rem', gap: '1rem', minHeight: 0 }} className="lg:col-span-2">
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <h2 className="text-2xl font-bold">汽车过凹桥模型</h2>
          </div>
        ...
        {/* 状态横幅 */}
        <div className="p-3 rounded-lg flex items-center gap-2 text-sm border bg-purple-500/20 border-purple-500/50 text-purple-200">
          <Info size={18} className="shrink-0" />
          <p>
            <strong>当前状态：{statusText}</strong>。
            (当前体感重量为正常时的 {(weightRatio*100).toFixed(0)}%，速度越快，超重感越强！)
          </p>
        </div>
        ...
        <div className="flex-1 min-h-0 relative">
          ...
          <FreeBodyDiagram width={1200} height={800} showGrid={false}>
            {/* Concave Bridge */}
            <path 
              d={`M ${CENTER_X - 400} 400 Q ${CENTER_X} ${600 + (80000/rPx)} ${CENTER_X + 400} 400`} 
            ...
            {/* Centripetal Force Indicator (purple dashed, UP) */}
            <ForceArrow 
              x={objX + 40} y={objY} 
              magnitude={clip(centripetalForce)} angle={270} 
            ...
            <g transform={`translate(${CENTER_X + 80}, ${objY - 80})`}>
              ...
              <text x="10" y="25" fill="#a855f7" fontSize="16" fontFamily="monospace">
                F_向 = N - mg = {formatPhysicsValue(centripetalForce)} N
              </text>
            </g>
      ...
      <aside style={{ display: 'flex', flexDirection: 'column', overflowY: 'auto', backgroundColor: 'rgba(15,23,42,0.6)', borderLeft: '1px solid rgba(255,255,255,0.08)' }} className="lg:col-span-1">
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <ParameterSlider label="凹桥半径" symbol="r" unit="m" value={radius} min={20} max={100} step={5} onChange={setRadius} />
          </div>
        </div>
        ...
        <div style={{ padding: '1.5rem', backgroundColor: 'rgba(59,130,246,0.05)', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <h4 className="font-bold flex items-center gap-2 mb-3 text-blue-400 text-sm">
            <Info size={15} /> 物理要点：凹桥超重
          </h4>
          <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.7)', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            <p className="text-yellow-300"><strong>易错提醒：</strong>"超重"也不是重力变大了，而是支持力 N 大于重力 mg，导致体感变重！</p>
            <p><strong>受力分析：</strong>在凹桥最低点，圆心在上方，合力必须向上提供向心力。<br/><InlineMath math="N - mg = m \frac{v^2}{r}" /></p>
            <p><strong>支持力变化：</strong><br/><InlineMath math="N = mg + m \frac{v^2}{r}" /><br/>速度 <InlineMath math="v" /> 越大，所需向心力越大，支持力 <InlineMath math="N" /> 就必须越大来凑够向心力。</p>
            <p><strong>危险提示：</strong>凹桥容易导致汽车悬挂系统承受过大压力而损坏，或驾驶员承受较大 G 力。</p>
          </div>
        </div>
        ...
        <div style={{ marginTop: 'auto', padding: '1.5rem', backgroundColor: 'rgba(255,255,255,0.04)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            {[
              ...
              { label: '超重系数 (N/mg)', value: `${weightRatio.toFixed(2)}`, color: '#facc15' },
            ].map((row) => (
